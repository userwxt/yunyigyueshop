//创建推荐商品
$.get(
    "./json/recommend.json?v=" + Math.random(),
    function (res) {
      ...
      for (var i = 0; i < res.data["count"]; i++) {
       ...
       <a  class="p-inner link" data="${p_id}"><img src="./img/${product[i]["products"]["coverIdStr"]}.jpg" alt=""></a>
       ...
       <p>¥<em>${minPrice}</em></p></div> `);
        if (minPrice !== o_price) {
          $($("#rec-list .p-inner")[i]).append(
            `<span class="discount"><i class="m-p">&yen;${minPrice}</i><i class="dis">&yen;${o_price}</i></span>`
          );
        }
      }
      //给每一个商品添加点击事件
      $("#rec-list .link").on("click", function () {
  ...
  // 创建热门商品
$.get(
    "./json/hotgoods.json?v=" + Math.random(),
    function (res) {
      ...
      for (var i = 0; i < cou; i++) {
        if (i >= res.data["count"]) {
          $("#hot-list").append(
            `<div class="product" style="visibility: hidden;flex: 0 0 23.8%;height: 382px;"></div>`
          );
        } else {
          p_id = product[i]["id"];
          ...
          $("#hot-list").append(`<div class="product" data="${p_id}">
                 ...
                 <img src="./img/${product[i]["coverIdStr"]}.jpg" alt="">
                 </a>
          ...
          if (minPrice !== o_price) {
            $($("#hot-list .p-inner")[i]).append(
              `<span class="discount"><i class="m-p">&yen;${minPrice}</i><i class="dis">&yen;${o_price}</i></span>`
            );
          }
          if (v_price) {
            v_price = product[i]["vipMinPrice"];
            $($("#hot-list .cost")[i]).append(
              `<em style="font-size: 15px; color: rgb(254, 103, 46);">(黑胶VIP &yen;${v_price})</em>`
            );
          }
        }
      }
